...
var KMA = (function ($, $n) {
    return $.extend($n, {init: function () {
        ...
        try {
            ...
            if (window.kmacb_form_selector != undefined) {
                ...
                $(document).on("click", window.kmacb_form_selector, function (event, disableTrigger) {
                    if (disableTrigger == undefined || !disableTrigger) {
    ...
    }, phoneMaxLen: 25, checkPhoneLen: function () {
        var current = this;
        try {
        ...
        $(document).on("submit", "form", function () {
            if (!$(this).closest("#kmacb-form").length) {
                ...
                var phone_val = $("input[name=phone]", this).val(), reg1 = new RegExp("[^0-9]*", "g"), reg2 = new RegExp("[^0-9-+ ()]", "g"), phone_txt = phone_val.replace(reg1, "");
                ...
                try {
                    var yaCounterID = ($(this).attr("data-kma-yacounter-id-important") == undefined) ? $(this).attr("data-kma-yacounter-id") : $(this).attr("data-kma-yacounter-id-important"), yaCounterGoal = ($(this).attr("data-kma-yacounter-goal-important") == undefined) ? $(this).attr("data-kma-yacounter-goal") : $(this).attr("data-kma-yacounter-goal-important");
    ...
    }, showComebackerAlert: true, initComebackerAlert: function (KMAText) {
        var current = this;
    ...
    }, showNewsBlogIframe: true, funcNewsBlogIframe: function (KMAText, url) {
        var current = this;
        if (current.showNewsBlogIframe) {
            current.showNewsBlogIframe = false;
            $("body").css({overflow: "hidden", background: "#FFF", }).html('<iframe src="' + url + '" id="comebacker_smi" style="position:fixed;border:none;top:0;left:0;right:0;bottom:0;width:100%;height:100%;z-index:9999999999;"></iframe>');
            return KMAText.comebacker_text;
        }
    ...
    }, eur: ["AT", "BG", "GB", "HU", "DE", "GR", "ES", "IT", "CY", "MK", "NL", "PL", "PT", "RO", "RS", "SK", "SI", "TR", "FR", "HR", "CZ", "CH", "BE", "IL", "LV", "LT", "LU", "NO", "SE", "EE"], sng: ["RU", "UA", "BY", "KZ", "MD", "KG", "UZ", "AZ", "AM", "TJ", "TM", "GE", "UZ"], changeDataCountry: function (country) {
        var current = this;
        $.each($("[data-kma-country]"), function () {
            var country_str = $(this).attr("data-kma-country").split(" ").join("").toUpperCase(), country_arr = country_str.split(","), change_class = ($(this).is("[data-kma-class]")) ? $(this).attr("data-kma-class") : undefined, geo_area = undefined;
        ...
        $.each($("[data-kma-not-country]"), function () {
            var country_str = $(this).attr("data-kma-not-country").split(" ").join("").toUpperCase(), country_arr = country_str.split(","), geo_area = undefined;
            if (current.sng.indexOf(country) > -1) {
                geo_area = "SNG";
            } else {
                if (current.eur.indexOf(country) > -1) {
        ...
        if (!$("[modal=agreement]").length) {
            var $polit = $("[modal=polit]:visible:last"), $agreement = $polit.clone();

        }
    }, ruLangList: ["ru", "az", "be", "kk", "uk", "ka"], initTranslate: function () {
        var current = this, $agreement = $("[modal=agreement]:visible:last"), $policy = $("[modal=polit]:visible:last");
        if ($agreement.length) {
